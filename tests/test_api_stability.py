...
def test_api_stability_clean():
    ...
    assert len(sig.parameters) == 1, "td.clean should only take 'data' to maintain API simplicity."
    assert sig.return_annotation.__name__ == "CleanResult" if hasattr(sig.return_annotation, "__name__") else str(sig.return_annotation).endswith("CleanResult")
...
def test_api_stability_inspect():
    ...
    assert sig.return_annotation is Any or sig.return_annotation == "Any" or sig.return_annotation == inspect.Parameter.empty or str(sig.return_annotation) == "typing.Any"
