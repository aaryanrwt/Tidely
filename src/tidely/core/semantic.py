...
class SemanticEngine:
    ...
    def __init__(self):
        ...
        self.patterns = {
            ...
            "UUID": re.compile(r"^[a-fA-F0-9]{8}-[a-fA-F0-9]{4}-[1-5][a-fA-F0-9]{3}-[89abAB][a-fA-F0-9]{3}-[a-fA-F0-9]{12}$"),
    ...
    def infer(self, df: Any, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for col, col_meta in metadata.get("columns", {}).items():
            ...
            if any(t in dtype for t in ("object", "string", "str")):
                ...
                uuid_matches = sum(1 for val in sample_list if isinstance(val, str) and self.patterns["UUID"].match(val.strip()))
                ...
                if uuid_rate > 0.8:
                    ...
                    if best_rate >= 0.5:
                        ...
                        if best_match == "Email":
                            recommended_cleaning = "Normalize to lowercase, strip whitespaces"
                            risk_score = 0.1
                        elif best_match == "Phone":
                            recommended_cleaning = "Strip non-digits, format international"
                            risk_score = 0.2
                        elif best_match == "ZIP Code":
                            recommended_cleaning = "Format to 5-digit ZIP"
                            risk_score = 0.1
                        elif best_match == "Boolean":
                            recommended_cleaning = "Cast truthy/falsy to bool primitives"
                            risk_score = 0.0
                        elif best_match == "Currency":
                            recommended_cleaning = "Extract numeric amount, normalize symbol"
                            risk_score = 0.1
                        elif best_match == "JSON":
                            recommended_cleaning = "Standardize JSON quotes and indentations"
                            risk_score = 0.3
                        elif best_match == "DNA Sequence":
                            recommended_cleaning = "None (DNA sequence preserved exactly)"
                            risk_score = 0.0
                    else:
                        # Fallback heuristic rules
                        if "address" in col_lower or any(kw in col_lower for kw in ("street", "road", "ave", "drive", "st", "lane", "ln")):
            ...
            # Numeric or Date parsing rules
                ...
                if vals:
                    min_val = min(vals)
                    max_val = max(vals)
                    if "lat" in col_lower and -90 <= min_val <= 90 and -90 <= max_val <= 90:
                        inferred_type = "Latitude"
                        confidence = 0.95
                        recommended_cleaning = "Clip to range [-90, 90]"
                    elif "lon" in col_lower and -180 <= min_val <= 180 and -180 <= max_val <= 180:
                        inferred_type = "Longitude"
                        confidence = 0.95
                        recommended_cleaning = "Clip to range [-180, 180]"
                    elif "int" in dtype and col_meta.get("unique_count", 0) == col_meta.get("total_count", 0):
                        inferred_type = "ID/Key"
                        confidence = 1.0
                        recommended_cleaning = "None"
                        ...
                        inferred_type = "Price"
                        confidence = 0.9
                        recommended_cleaning = "Standardize decimals, treat negative values"
                    else:
