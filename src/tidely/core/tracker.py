...
class OutcomeTracker:
    ...
    def track(self, cleaned_df: Any, autofixes: List[Dict[str, Any]], warnings: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for action in autofixes:
            ...
            if cat == "Duplicate Rows":
                formatted_fixes.append(f"Duplicate Rows removed.\n    Why: {why}\n    Impact: {impact}")
            else:
                formatted_fixes.append(f"{cat} applied.\n    Why: {why}\n    Impact: {impact}")
        ...
        for warn in warnings:
            ...
            why = warn.get("why", "")
            formatted_warnings.append(f"{cat} in '{col}' left unchanged (Confidence {conf}% < 95%).\n    Why: {why}")
