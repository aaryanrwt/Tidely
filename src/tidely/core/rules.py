...
def make_outlier_iqr_rule(column: str, threshold: float = 1.5) -> Callable[[pl.DataFrame], pl.DataFrame]:
...
def make_outlier_zscore_rule(column: str, threshold: float = 3.0) -> Callable[[pl.DataFrame], pl.DataFrame]:
...
def make_coordinate_clip_rule(column: str, is_lat: bool) -> Callable[[pl.DataFrame], pl.DataFrame]:
...
def make_replace_null_placeholders_rule(column: str) -> Callable[[pl.DataFrame], pl.DataFrame]:
    ...
    def _rule(df: pl.DataFrame) -> pl.DataFrame:
        ...
        return df.with_columns(
            pl.when(
                pl.col(column).cast(pl.String).str.strip_chars().is_in(["?", "N/A", "n/a", "null", "NULL", "NaN", "nan"])
            )
            .then(None)
