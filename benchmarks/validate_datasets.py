...
def generate_healthcare_dataset(rows: int = 100000) -> pd.DataFrame:
    ...
    df = pd.DataFrame({
        ...
        "doctor_name": np.random.choice(["Dr. House", "Dr. Watson", "Dr. Grey", None], rows),
        "systolic_bp": np.random.normal(120, 15, rows),
        "diastolic_bp": np.random.normal(80, 10, rows),
        "pulse": np.random.normal(72, 8, rows),
        "visit_date": np.random.choice(["2025-01-01", "02/14/2025", "invalid_date", None], rows)
    })
    # Inject nulls
...
def generate_finance_dataset(rows: int = 100000) -> pd.DataFrame:
    ...
    df = pd.DataFrame({
        ...
        "credit_card": [f"4111-1111-1111-{np.random.randint(1000, 9999)}" for _ in range(rows)],
...
def generate_ecommerce_dataset(rows: int = 100000) -> pd.DataFrame:
    ...
    df = pd.DataFrame({
        ...
        "sku": [f"SKU-{np.random.randint(100, 999)}-{np.random.choice(['A', 'B'])}" for _ in range(rows)],
