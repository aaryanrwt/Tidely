...
ROOT_DIR = (
    r"C:\Users\Aaryan Rawat\Downloads\amazon+product+and+google+locations+reviews"
)
...
def main():
    ...
    for filepath in discovered_files:
        ...
        try:
            ...
            nulls_before = sum(
                profile._df_ref[col].null_count() for col in profile._df_ref.columns
            )
            ...
            print(
                f"[{filename}] Complete. Duration: {duration:.1f}ms, Mem: {mem_overhead:.1f}MB, Rows: {initial_rows} -> {output_rows}"
            )
            ...
            if dup_rows_before > 0 and dup_rows_after > 0:
                issues.append(
                    f"Failed to remove all duplicate rows: {dup_rows_after} remaining"
                )
            if nulls_numeric_after > 0:
                issues.append(
                    f"Failed to impute all numeric nulls: {nulls_numeric_after} remaining"
                )
    ...
    report_lines.extend(["", "## Bugs Found & Handled", ""])

    if not bugs_found:
        report_lines.append(
            "✓ No bugs detected during stress testing campaign! 100% of functional checks passed."
        )
    else:
