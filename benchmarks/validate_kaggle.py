...
VGG16_ROOT = (
    r"C:\Users\Aaryan Rawat\.cache\kagglehub\datasets\crawford\vgg16\versions\2"
)
...
def main():
    ...
    pokemon_files = [
        os.path.join(POKEMON_ROOT, f)
        for f in os.listdir(POKEMON_ROOT)
        if f.endswith(".json")
    ]
    print(
        f"Found {len(pokemon_files)} JSON battle files. Flattening top 10 episodes..."
    )
    ...
    with open(vgg_json, encoding="utf-8") as f:
        vgg_data = json.load(f)

    records_vgg = [
        {"index": int(k), "class_id": v[0], "class_name": v[1]}
        for k, v in vgg_data.items()
    ]
    ...
    try:
        ...
        assert len(diff) == 0, (
            "IMDb review text was altered or corrupted during cleaning!"
        )

        # Stress test corrupted
        corr_imdb = generate_corrupted_df(
            imdb_df.head(1000)
    ...
    if not bugs_found:
        report_lines.append(
            "✓ No bugs detected during Kaggle campaign! 100% of pipeline checks and stress assertions passed."
        )
    else:
