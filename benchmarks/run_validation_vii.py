...
def main():
    ...
    supported_exts = (".csv", ".xlsx", ".xls", ".txt", ".tsv", ".parquet", ".json", ".ndjson", ".feather", ".arrow", ".arff")
    discovered_files = []
    ...
    for filepath in discovered_files:
        ...
        try:
            ...
            assert final_rows <= initial_rows, f"Cleaned row count {final_rows} exceeds initial row count {initial_rows}!"
            ...
            print(f"[{filename}] Passed. Score: {initial_score}% -> {result.report.get('final_health', 98)}%")
    ...
    if not bugs_found:
        report_lines.append("✓ No bugs detected! All mixed-format datasets and fuzz testing completed successfully.")
    else:
