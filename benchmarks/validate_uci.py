...
def main():
    ...
    print(f"Adult Income '?' presence - Before: {has_question_marks_before} | After: {has_question_marks_after}")
    assert not has_question_marks_after, "Adult Income dataset should not contain '?' missing placeholders post-clean!"
    ...
    splice_df["joined_dna_sequence"] = splice_df[base_cols].astype(str).agg("".join, axis=1)
    ...
    for col in dna_cols:
        ...
        assert inferred == "DNA Sequence", f"DNA sequence column '{col}' misclassified as: {inferred}"
        ...
        assert set(original_seqs) == set(cleaned_seqs), f"DNA sequences in column '{col}' were corrupted or normalized!"
