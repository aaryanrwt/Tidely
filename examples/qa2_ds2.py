...
def process_qa2_ds2():
    print("Downloading Dataset 2: Supermarket...")
    try:
    ...
    with open("qa2_ds2_baseline.json", "w") as f:
    ...
    with open("qa2_ds2_inspection.json", "w") as f:
        json.dump(report_p2, f, indent=4)
        
    # PHASE 3: Tidely Cleaning
    df_clean = df.copy()
    result = td.clean(df_clean)
    with open("qa2_ds2_cleaning.json", "w") as f:
    ...
    print(f"DS2 Pandas Time: {time_pd:.4f}s")
    print(f"DS2 Tidely Time: {time_td:.4f}s")
    print(f"DS2 Ratio:       {time_td / max(time_pd, 0.0001):.2f}x")
        
    print("Completed QA2 DS2 Phases 1, 2, 3, 7.")
